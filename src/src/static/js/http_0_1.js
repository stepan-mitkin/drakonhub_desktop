...
function handleBasicStatusChange(request, resolve) {
    if (request.readyState === 4) {
        if (request.status === 200 || request.status === 204) {
            resolve(true);
        } else {
            console.error('handleBasicStatusChange: an error has occurred', request);
            resolve(false);
        }
    }
}
...
function uploadFileToServer(url, name, file) {
    return new Promise(resolve => {
        ...
        request.onreadystatechange = () => {
            handleBasicStatusChange(request, resolve);
        };
