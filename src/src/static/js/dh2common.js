...
async function saveAsSvg(widget) {
    ...
    if (window.padBridge && window.padBridge.exportSvg) {
        await window.padBridge.exportSvg(filename, image, mime);
        widgets.showGoodSnack(translate('Saved') + ': ' + filename);
    } else {
...
function setTitle(title) {
    var fullTitle;
    if (title) {
        fullTitle = title + ' | ' + gconfig.appName;
    } else {
        fullTitle = gconfig.appName;
    }
    if (window.padBridge && window.padBridge.setTitle) {
        window.padBridge.setTitle(fullTitle);
    } else {
        html.setTitle(fullTitle);
    }
}
