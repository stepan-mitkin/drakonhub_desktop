...
function DrakonCanvas() {
    ...
    function exportJson() {
        var _collection_2, copy, diagram, id, item, src;
    ...
    function getLoadedImages() {
        var _collection_2, id, image, result;
        result = {};
        _collection_2 = self.images;
        for (id in _collection_2) {
            image = _collection_2[id];
    ...
    function showPaste() {
        var _selectValue_2, clipboard;
        tracing.trace('DrakonCanvas.showPaste');
        if (!isReadonlyImpl(self)) {
            clipboard = getClipboardClone(self);
            if (clipboard) {
                _selectValue_2 = clipboard.type;
                if (_selectValue_2 === 'case' || _selectValue_2 === 'branch' || _selectValue_2 === 'block' || _selectValue_2 === 'mind') {
...
function OuterCrawler_create(visuals, sub) {
    ...
    function* OuterCrawler_main() {
        var _branch_, _event_, _selectValue_2, edge, head, leftMost, next, output, tail;
        _branch_ = 'Init';
        while (true) {
            switch (_branch_) {
            case 'Init':
...
function SelectBehavior_create(widget) {
    ...
    function* SelectBehavior_main() {
        var _branch_, _eventType_, _event_, _selectValue_2, currentSocket, cursor, deltaX, deltaY, dragTarget, ear, evt, pos, prim, scroller, startX, startY;
        _branch_ = 'Idle';
        while (true) {
            switch (_branch_) {
            case 'Idle':
...
function blockSelect(widget) {
    var _branch_, _collection_2, id, node, prim, selection, selectionIds, snode, visuals;
...
function buildContextMenuForPrim(widget, prim, evt) {
    var _selectValue_2, _selectValue_4, edge, menu, node, selected, underlying, visuals;
    visuals = widget.visuals;
    if (prim) {
        _selectValue_2 = prim.elType;
        if (_selectValue_2 === 'node') {
            ...
            if (_selectValue_2 === 'edge') {
                edge = getEdge(visuals, prim.id);
                _selectValue_4 = edge.role;
                if (_selectValue_4 === 'parceiling') {
                    selectPrim(widget, prim.id);
                    menu = buildParCeilMenu(widget, edge);
                } else {
                    if (_selectValue_4 === 'down') {
                        ensureSelectedOne(widget, prim);
                        menu = buildDownEdgeMenu(widget, edge);
                    } else {
                        if (_selectValue_4 === 'mind-child') {
                            selectPrim(widget, prim.id);
                            menu = buildMindEdgeMenu(widget, edge);
                        } else {
                            if (_selectValue_4 === 'ceil') {
                                selectPrim(widget, prim.id);
                                menu = buildCeilEdgeMenu(widget, edge);
                            } else {
                                if (_selectValue_4 === 'selectceil') {
                                    selectPrim(widget, prim.id);
                ...
                if (_selectValue_2 === 'free') {
                    ...
                    if (_selectValue_2 === 'nugget') {
                        ...
                        if (_selectValue_2 === 'connection') {
                            if (!isSelected(widget, prim.id)) {
                                selectPrim(widget, prim.id);
                            }
                            menu = buildConnectionMenu(widget, prim);
                        } else {
                            if (_selectValue_2 === 'handle') {
...
function buildDiagramModel(widget, diagram) {
    var _collection_2, branch, end, idNum, item, itemId, model, nextId, type;
    diagram.initial = [];
...
function buildMenuByTypeMind(widget, prim, node) {
    ...
    if (isReadonlyImpl(widget)) {
        ...
        menu.push({ type: 'separator' });
        _selectValue_2 = getTType(prim);
        if (_selectValue_2 === 'horizontal') {
...
function buildMindTree(visuals) {
    var _collection_2, _collection_5, _collection_8, id, node;
    _collection_2 = visuals.nodes;
    for (id in _collection_2) {
        node = _collection_2[id];
        node.children = [];
    ...
    _collection_5 = visuals.nodes;
    for (id in _collection_5) {
        node = _collection_5[id];
        if (node.parent) {
            node.parent.children.push(node);
        }
    }
    _collection_8 = visuals.nodes;
    for (id in _collection_8) {
        node = _collection_8[id];
...
function buildSkewers(visuals) {
    var _collection_2, _collection_5, id, node;
    _collection_2 = visuals.nodes;
    for (id in _collection_2) {
        node = _collection_2[id];
...
function canDelete(visuals, node) {
    ...
    if (_selectValue_2 === 'junction' || (_selectValue_2 === 'header' || _selectValue_2 === 'arrow-loop' || _selectValue_2 === 'address' || _selectValue_2 === 'end')) {
        return false;
    } else {
        if (_selectValue_2 === 'branch') {
...
function canHaveDuration(node) {
    var _selectValue_2;
    _selectValue_2 = node.type;
    if ((_selectValue_2 === 'action' || (_selectValue_2 === 'question' || _selectValue_2 === 'select' || _selectValue_2 === 'insertion' || _selectValue_2 === 'simpleinput' || _selectValue_2 === 'simpleoutput' || _selectValue_2 === 'input' || _selectValue_2 === 'output' || _selectValue_2 === 'shelf' || _selectValue_2 === 'process')) && !node.side) {
        return true;
    } else {
        return false;
    }
...
function canSelectNode(node) {
    var _selectValue_2;
    _selectValue_2 = node.type;
    if (_selectValue_2 === 'end' || _selectValue_2 === 'duration' || _selectValue_2 === 'header' || (_selectValue_2 === 'branch' || _selectValue_2 === 'address')) {
        return false;
    } else {
        if (_selectValue_2 === 'junction') {
            if (node.subtype === 'parbegin') {
                return true;
            } else {
                return false;
            ...
            if (_selectValue_2 === 'action' && node.id === 'params') {
                return false;
            } else {
                return true;
            }
...
function copyCore(widget) {
    var _selectValue_2, all, elements, nodes;
    nodes = getNodesFromSelection(widget);
    _selectValue_2 = nodes.length;
    if (_selectValue_2 === 0) {
...
function createSocketFromEdge(visuals, edge, op, type, imageData) {
    var Min, _selectValue_2, box, endPos, head, hh, hw, length, radius, socket, startPos, tail, th, tw, x, x1, x2, y, y1, y2;
    ...
    _selectValue_2 = edge.role;
    if (_selectValue_2 === 'floor') {
        socket.target = getFloorTarget(visuals, edge);
    } else {
        if (_selectValue_2 === 'parceiling') {
...
function deleteOne(widget, node) {
    var _selectValue_2, edits, nodes;
    edits = [];
    if (canDelete(widget.visuals, node)) {
        if (isMindIcon(widget, node)) {
            ...
            if (_selectValue_2 === 'question') {
                ...
                if (_selectValue_2 === 'loopbegin') {
                    ...
                    if (_selectValue_2 === 'loopend') {
                        edits = deleteLoop(widget, node.loopStart);
                    } else {
                        if (_selectValue_2 === 'params') {
...
function deleteOrphanImages(widget, edits) {
    var _collection_2, id, image, images2;
    images2 = {};
    _collection_2 = widget.images;
    for (id in _collection_2) {
        image = _collection_2[id];
        if (image.refCount === 0) {
            deleteItemCore(edits, id);
...
function drawEars(visuals, ctx) {
    var _collection_2, border, box, config, ears, fill, key, radius, selectedBox;
    fill = '#00ff00';
    border = 'black';
    ears = visuals.ears;
    config = visuals.config;
    if (ears) {
        ...
        if (ears.selected) {
            ...
            _collection_2 = ears.boxes;
            for (key in _collection_2) {
                box = _collection_2[key];
...
function drawEdge(widget, edge, ctx) {
    var _selectValue_2, color, config, endPos, length, low, radius, showMeat, socketY, startPos, thickness, visuals, x1, x2, y1, y2;
    visuals = widget.visuals;
    config = visuals.config;
    x1 = getX(edge.head);
    y1 = getY(edge.head);
    ...
    if (edge.vertical) {
        ...
        if (edge.role === 'parceiling') {
            ...
            _selectValue_2 = edge.role;
            if (_selectValue_2 === 'arrow') {
                drawArrowHead(ctx, color, x1 + thickness, y1, Math.PI);
            } else {
                if (_selectValue_2 === 'rarrow') {
...
function drawFreeNuggetAndHandles(widget, ctx) {
    var _collection_2, _collection_5, common, element, id, showNugget, type, visuals, x, y;
    showNugget = false;
    visuals = widget.visuals;
    ctx.setLineDash([]);
    clearShadow(ctx);
    ...
    _collection_2 = widget.selection.prims;
    for (id in _collection_2) {
        type = _collection_2[id];
        if (type === 'free') {
            element = getFree(visuals, id);
            if (!(element.type === 'group-duration')) {
    ...
    _collection_5 = widget.selection.prims;
    for (id in _collection_5) {
        type = _collection_5[id];
        if (type === 'free') {
            element = getFree(visuals, id);
...
function findClosestNode(widget, filter, srcNode, vertical) {
    var _collection_2, best, current, distance, id, node;
    best = undefined;
    distance = 10000000000;
    _collection_2 = widget.visuals.nodes;
    for (id in _collection_2) {
        node = _collection_2[id];
...
function findHead(selection, byId) {
    var _collection_2, id, item, type;
    _collection_2 = selection.prims;
    for (id in _collection_2) {
        type = _collection_2[id];
        if (!(type === 'node')) {
            return undefined;
...
function findLeftLinks(visuals, skewer) {
    var _branch_, _selectValue_2, boundary, corner, finish, head, hskewer, left, metre, node, rightJ, start, tail;
    _branch_ = 'Start';
    while (true) {
        switch (_branch_) {
        case 'Start':
...
function findLianaSource(visuals, prim) {
    var _selectValue_2, beneath, edge, node;
    _selectValue_2 = prim.elType;
    if (_selectValue_2 === 'node') {
        node = getNode(visuals, prim.id);
        if (node.arrow) {
...
function getEarRole(ears) {
    var _selectValue_2;
    _selectValue_2 = ears.selected;
    if (_selectValue_2 === 'up' || _selectValue_2 === 'down') {
        return 'vertical';
    } else {
        return 'horizontal';
    }
...
function getFreeFromSelection(widget) {
    var _collection_2, elType, id, visuals;
    visuals = widget.visuals;
    _collection_2 = widget.selection.prims;
    for (id in _collection_2) {
        elType = _collection_2[id];
        if (elType === 'free') {
...
function getNodesFromSelection(widget) {
    var _collection_2, elType, id, node, result, visuals;
    visuals = widget.visuals;
    result = [];
    _collection_2 = widget.selection.prims;
    for (id in _collection_2) {
...
function getOrCreateUndoEdit(self, diagramId, diagram, sender, canReuseUndo) {
    ...
    if (diagramId) {
        if (diagramId in self.undoBuffers && canReuseUndo) {
            ...
            undoBuffer.connect(diagram, sender);
        } else {
...
function getSelectedPrims(widget) {
    var _collection_2, connection, elType, element, id, node, prim, prims, visuals;
    visuals = widget.visuals;
    prims = [];
    _collection_2 = widget.selection.prims;
    for (id in _collection_2) {
...
function hitEars(visuals, pos) {
    var _collection_2, box, key;
    if (visuals.ears) {
        _collection_2 = visuals.ears.boxes;
        for (key in _collection_2) {
            box = _collection_2[key];
            if (hitBox(box, pos.x, pos.y)) {
                return key;
...
function isSimpleItem(node) {
    var _selectValue_2;
    _selectValue_2 = node.type;
    if (_selectValue_2 === 'header' || (_selectValue_2 === 'end' || (_selectValue_2 === 'junction' || _selectValue_2 === 'select' || _selectValue_2 === 'question' || _selectValue_2 === 'address')) || _selectValue_2 === 'case' && firstCase(node.select) === node) {
        return false;
    } else {
        return true;
    }
...
async function loadImages(widget) {
    var _collection_2, diagram, id, image, item;
...
function nodeFromItem(visuals, item) {
    var _selectValue_2, content, node;
    if (item.type === 'end') {
...
function paintCore(widget, factor, zoom, translateX, translateY, width, height) {
    ...
    ctx.setLineDash([]);
    _collection_2 = visuals.edges;
    for (id in _collection_2) {
        edge = _collection_2[id];
        drawEdge(widget, edge, ctx);
    }
    _collection_8 = widget.selection.prims;
    for (id in _collection_8) {
        type = _collection_8[id];
        if (type === 'edge') {
    ...
    ctx.setLineDash([]);
    _collection_11 = widget.selection.prims;
    for (id in _collection_11) {
        type = _collection_11[id];
        if (type === 'node') {
...
function pasteInSocket(widget, socket, images) {
    var _selectValue_2, _selectValue_4, clipboard, ctype, edits, payload;
    edits = [];
    clipboard = getClipboardClone(widget);
    if (clipboard) {
        payload = clipboard.content;
        ...
        if (ctype === 'mind') {
            _selectValue_4 = socket.type;
            if (_selectValue_4 === 'mind-before') {
                edits = mindBeforeInsert(widget, socket, payload);
            } else {
                if (_selectValue_4 === 'mind-after') {
            ...
            _selectValue_2 = socket.type;
            if (_selectValue_2 === 'block') {
                ...
                if (_selectValue_2 === 'case') {
                    ...
                    if (_selectValue_2 === 'duration') {
                        ...
                        if (_selectValue_2 === 'first-case') {
                            if (ctype === 'case') {
                                edits = firstCaseInsertCore(widget, socket.node, payload.items[0]);
                            }
                        } else {
                            if (_selectValue_2 === 'branch' && socket.type === ctype) {
...
function planNextSteps(visuals, stack, node) {
    var _selectValue_2, next1, next2;
    _selectValue_2 = node.type;
    if (_selectValue_2 === 'select') {
        ...
        if (_selectValue_2 === 'question') {
            ...
            if (_selectValue_2 === 'arrow-loop') {
                ...
                if (_selectValue_2 === 'junction' && node.subtype === 'parbegin') {
...
function positionDurations(visuals) {
    var _collection_2, dur, edge, id, metre, node, x;
    metre = visuals.config.metre;
    _collection_2 = visuals.nodes;
    for (id in _collection_2) {
        node = _collection_2[id];
...
function positionLevels(visuals) {
    var _collection_11, _collection_2, _collection_5, _collection_8, distance, down, h, id, jun, leftHeight, level, lowest, max, metre, node;
    metre = visuals.config.metre;
    _collection_2 = visuals.nodes;
    for (id in _collection_2) {
        node = _collection_2[id];
    ...
    max = 0;
    _collection_5 = visuals.levels;
    for (id in _collection_5) {
        level = _collection_5[id];
        max = Math.max(max, level.coord);
    }
    _collection_8 = visuals.levels;
    for (id in _collection_8) {
        level = _collection_8[id];
        level.coord = max - level.coord;
    }
    _collection_11 = visuals.nodes;
    for (id in _collection_11) {
        node = _collection_11[id];
...
function positionSkewers(visuals) {
    var _collection_2, id, left, skewer;
    _collection_2 = visuals.skewers;
    for (id in _collection_2) {
        skewer = _collection_2[id];
...
function reflowIcon(visuals, node) {
    var _selectValue_2, config, size;
    config = visuals.config;
    size = commitCore(visuals, node, node.w * 2);
    setNodeSize(config, node, size);
    node.contentHeight = size.height;
    _selectValue_2 = node.type;
    if (_selectValue_2 === 'branch' || (_selectValue_2 === 'case' || _selectValue_2 === 'address')) {
...
async function runInsertAction(widget, socket) {
    var _selectValue_2, action, edits, images, prim;
    images = {};
    _selectValue_2 = socket.op;
...
function selectCluster(widget, headNode, node) {
    var _collection_4, _selectValue_2, context, id, included, info, inode, result, start;
    _selectValue_2 = node.type;
    if (_selectValue_2 === 'select') {
        start = node;
        context = {
        ...
        included = [];
        _collection_4 = context.nodes;
        for (id in _collection_4) {
            info = _collection_4[id];
            if (info.include) {
...
function setFontToToken(token, fontObj) {
    var _selectValue_2, font, font2;
    font = fontObj.font;
    _selectValue_2 = token.type;
    if (_selectValue_2 === 'strong') {
        font2 = {};
        Object.assign(font2, fontObj);
        font2.weight = 'bold';
        token.font = cssFontToString(font2);
    } else {
        if (_selectValue_2 === 'em') {
            font2 = {};
            Object.assign(font2, fontObj);
            font2.style = 'italic';
            token.font = cssFontToString(font2);
        } else {
            if (_selectValue_2 === 'sem') {
                font2 = {};
                Object.assign(font2, fontObj);
                font2.style = 'italic';
                font2.weight = 'bold';
...
function setSameHeightMind(visuals) {
    var _collection_2, id, node, ttype;
    _collection_2 = visuals.nodes;
    for (id in _collection_2) {
        node = _collection_2[id];
        ttype = getTType(node);
...
function showLianaSockets(widget, prim) {
    var _branch_, _collection_11, _collection_14, _collection_2, _collection_5, _collection_8, downEdge, id, record, source, targetId, visuals;
    ...
    while (true) {
        ...
        case 'Outer-outer':
            if (source.outer) {
                _collection_14 = source.outer.outer;
                for (targetId in _collection_14) {
                    record = _collection_14[targetId];
        ...
        case 'Arrow pads':
            if (!source.arrow) {
                _collection_11 = source.outer.outerArrPads;
                for (id in _collection_11) {
                    downEdge = _collection_11[id];
            ...
            break;
        case 'Outer-inner':
            _collection_8 = source.outer.inner;
            for (targetId in _collection_8) {
                record = _collection_8[targetId];
        ...
        case 'Inner-outer':
            if (source.inner) {
                _collection_5 = source.inner.outer;
                for (targetId in _collection_5) {
                    record = _collection_5[targetId];
        ...
        case 'Inner-inner':
            if (isDegQuestion(source)) {
                _collection_2 = source.inner.inner;
                for (targetId in _collection_2) {
                    record = _collection_2[targetId];
...
function showMindInsertSockets(widget, op, type, imageData) {
    var _collection_2, id, node, visuals;
    visuals = widget.visuals;
    _collection_2 = visuals.nodes;
    for (id in _collection_2) {
        node = _collection_2[id];
...
function skipParBlock(node) {
    var _selectValue_2, counter;
    counter = 1;
    while (true) {
        node = getDown(node);
        _selectValue_2 = node.subtype;
        if (_selectValue_2 === 'parend') {
            counter--;
            if (counter === 0) {
                break;
