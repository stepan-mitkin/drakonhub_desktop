...
async function createNewDiagramCore(inNewWindow) {
    var file, newInfo;
    widgets.closePopup();
    ...
    if (newInfo) {
        ...
        file = await launcher.createDiagram(newInfo.type, newInfo.name, inNewWindow);
        unfreezeScreen();
...
async function editorCreateNew() {
    return await createNewDiagramCore(true);
}
...
async function startCreateNew() {
    return await createNewDiagramCore(false);
}
