function localserver(utils) {
  ...
  async function findFoldersByName(body) {
    var found = [];
    var needle = normalizeStringForSearch(body.needle);
  ...
  async function folderMatches(node, needle, found) {
    var lowName = normalizeStringForSearch(node.name);
  ...
  function createItemSearch(needleRaw, accurate) {
    var needle = normalizeStringForSearch(needleRaw);
    ...
    async function scanDiagram(node) {
      ...
      if (body.items) {
        for (var item of body.items) {
          if (item.type !== "image") {
            searchTextChunk(item.id, item.content, needle, found, accurate);
  ...
  function textContainsNeedle(text, needle, accurate) {
    var lowText = normalizeStringForSearch(text);
    if (accurate) {
  ...
  async function findFolders(body) {
    ...
    var lines = body.lines.map(function (line) {
      return normalizeStringForSearch(line);
    });
    await traverseFolders(ROOTID, function (node) {
  ...
  async function checkNameIsUnique(parentId, name, childId) {
    ...
    var low = normalizeStringForSearch(name);
    for (var child of children) {
      if (child.id === childId) {
        continue;
      }
      var lowChild = normalizeStringForSearch(child.name);
      if (lowChild === low) {
        return false;
      }
    }
  ...
  async function main() {
    ...
    if (window.padBridge.copyInitialFiles) {
      await window.padBridge.copyInitialFiles();
    }
  }
