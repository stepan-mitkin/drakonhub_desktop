...
function DrakonHubWidget() {
    ...
    function showInsertionSockets(type) {
        var _selectValue_199;
        _selectValue_199 = self.type;
        if (_selectValue_199 === 'drakon') {
            self.drakon.showInsertionSockets(type);
        } else {
            if (_selectValue_199 === 'graf') {
                if (type === 'action') {
...
async function startEditContent(widget, prim, ro) {
    var _branch_, _selectValue_201, canwidget, fonts, nameChecker, newContent, path, tr;
    _branch_ = 'Branch1';
    while (true) {
        switch (_branch_) {
        case 'Branch1':
            tr = widget.widgetSettings.translate;
            path = widget.widgetSettings.imagePath;
            _selectValue_201 = prim.type;
            if (_selectValue_201 === 'header') {
                if (ro) {
...
function detectDescChange(widget, edit) {
    var _collection_203, change, desc;
    desc = undefined;
    _collection_203 = edit.changes;
    for (change of _collection_203) {
...
function showPalette(context, launcher, value, onColorChosen) {
    var _collection_205, apply, bottom, chooseColor, chooseColorLight, closeAndUse, color, currentContainer, data, i, input, line, lineColor, lineContainer, lines, paletteWindow, recent, rect;
    ...
    _collection_205 = data.lines;
    for (line of _collection_205) {
        lineContainer = div({
            'padding-left': '5px',
            'padding-top': '1px',
            'padding-bottom': '1px',
...
function rebuildToolbar(widget) {
    var _selectValue_222, tr, type, typeCombo;
    tr = widget.widgetSettings.translate;
    ...
    if (widget.diagram) {
        ...
        if (widget.diagram.access === 'read') {
            ...
            _selectValue_222 = widget.diagram.type;
            if (_selectValue_222 === 'drakon') {
                ...
                if (_selectValue_222 === 'graf') {
                    updateMindButtons(widget);
                } else {
                    if (_selectValue_222 !== 'free') {
                        throw new Error('Unexpected case value: ' + _selectValue_222);
                    }
                    typeCombo = html.createElement('select');
                    typeCombo.style.width = '82px';
                    typeCombo.style.marginTop = '3px';
...
function updateFreeIconButtons(widget) {
    var _branch_, _selectValue_224, path, row3, tr;
    _branch_ = 'Clear';
    while (true) {
        switch (_branch_) {
        case 'Clear':
        ...
        case 'Common 2':
            ...
            _selectValue_224 = widget.typeCombo.value;
            if (_selectValue_224 === 'basic') {
                _branch_ = 'Exit';
            } else {
                if (_selectValue_224 === 'ui') {
                    _branch_ = 'UI';
                } else {
                    if (_selectValue_224 !== 'architect') {
...
function updateIconButtons(widget) {
    var _selectValue_226, path, row, row2, row3, row4, tr;
    tr = widget.widgetSettings.translate;
    path = widget.widgetSettings.imagePath;
    html.clear(widget.iconButtons);
...
function editHtml_create(left, top, header, oldContent, ro, path, tr) {
    ...
    function* editHtml_main() {
        var _branch_, _eventType_, _event_, buttonsDiv, cancel, contentDiv, dialog, hint, ok, popup, quill, result, safe;
